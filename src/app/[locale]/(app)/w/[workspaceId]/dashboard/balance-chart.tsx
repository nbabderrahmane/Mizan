    ...
    return (
        <Card className="col-span-4">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-base font-normal">{t("balanceHistory")}</CardTitle>
                <Tabs value={range} onValueChange={(v) => setRange(v as Range)} className="w-auto">
                    <TabsList className="grid w-full grid-cols-5 h-8">
                        ...
                        <TabsTrigger value="mtd" className="text-xs px-2 h-6">{new Date().toLocaleDateString(locale, { month: "short" })}</TabsTrigger>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data}>
                                ...
                                <YAxis
                                    ...
                                    tickLine={false}
                                    axisLine={false}
                                    tickFormatter={(value) => `${new Intl.NumberFormat(locale, { notation: "compact", compactDisplay: "short" }).format(value)}`}
                                />
                                <Tooltip
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="font-bold">
                                                                {new Intl.NumberFormat(locale, { style: 'currency', currency }).format(payload[0].value as number)}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
